...
def train_distill(epoch, train_loader, module_list, criterion_list, optimizer, opt,dataframetime,s,numofclass,tb):
    ...
    for idx, data in enumerate(train_loader):
        ...
        if torch.cuda.is_available():
            ...
            adv = LinfPGDAttack(model_s, loss_fn=nn.CrossEntropyLoss(),
                                eps=opt.eps, nb_iter=opt.max_iter, eps_iter=opt.eps_step, rand_init=True, clip_min=0., clip_max=1., targeted=opt.target)
            target = target.cuda()
            if opt.target != False:
        ...
        with torch.no_grad():
            if s != opt.input_dim:
                ...
                logit_t,feat_t = model_t(input_t,opt.is_sample)
                feat_t=feat_t.detach()
            else:
                logit_t,feat_t = model_t(input,opt.is_sample)
                feat_t=feat_t.detach()
        ...
        with torch.no_grad():
            if s != opt.input_dim:
                logit_tadv,f_tadv = model_t(adv_t,opt.is_sample)
                
            else:
                logit_tadv,f_tadv = model_t(adv,opt.is_sample)
    ...
    tb.add_scalar('Time per epoch',data_time.avg,epoch)
...
def validate(val_loader, model, criterion, num,opt,attack,numofclass):
    ...
    adversary = LinfPGDAttack(model, loss_fn=nn.CrossEntropyLoss(),
                                eps=opt.eps, nb_iter=opt.max_iter, eps_iter=opt.eps_step, rand_init=True, clip_min=0., clip_max=1., targeted=False)
    model.eval()
